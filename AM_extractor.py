...
def calc(j):
    ...
    while AM>=0:
        ...
        for elemento in actor:
            for cara in cluster:
                if(cc.similitudCoseno(cara,elemento)>AM):
                    count=count+1
        if(count>0):
            print('{} true per {}, {} vegades amb AM: {}'.format(clusters[j],actores[j],count,1-AM))
            k.append(count)
        else:
            k.append(0)
    return k
    ...
    for elemento in actor:
        for cara in cluster:
            if(cc.similitudCoseno(cara,elemento)>AM):
                count=count+1
    if(count>0):
        print('{} true per {}, {} vegades amb AM: {}'.format(clusters[0],actores[0],count,1-AM))
        x.append(1-AM)
        y.append(count)
    else:
        x.append(1-AM)
